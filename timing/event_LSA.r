...
setwd("~/Documents/Research/fMRI_data/Reading/Compute_data/TimingFiles/predictability/content2/lsa")
...
for (i in unique(mdata$RECORDING_SESSION_LABEL)) {
  ...
  if (nrow(sub1data) > 0) {
    ...
    write.table(sub1data, paste(i, "_lsa", ".txt", sep = ""), sep = "\t", na = "", col.names = FALSE, row.names = FALSE, quote = FALSE)
    }
}
